...
def update_market_status_cache(logger) -> str:
    ...
    Returns the exact market status (e.g., "OPEN" or "CLOSED"). If the API call fails,
    the function returns "UNKNOWN" and still records this state in Supabase.
    """
    url = "https://api.upstox.com/v2/market/status/nse"
    # Default to UNKNOWN – we only set a concrete status after a successful API response.
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=7) as response:
            if response.status == 200:
                ...
                api_status = raw_data.get("data", {}).get("status", "UNKNOWN")
                # Accept the status directly from the API (typically "OPEN" or "CLOSED").
                if api_status in ("OPEN", "CLOSED"):
                    status_result = api_status
    except Exception as e:
        logger.warning(
            "Upstox API unreachable. Defaulting market status to UNKNOWN. Error: %s", e
        )

...
class PortfolioEngine:
    ...
    def is_market_open(self) -> bool:
        ...
        # Define market timing boundaries in IST
    ...
    def update_from_candidates(self, candidates: pd.DataFrame | None = None) -> pd.DataFrame:
        ...
        """
        # Market Hours Guard: To strictly prevent entering positions out of hours, 
        ...
        if candidates is None:
            if supabase_client:
                try:
                    ...
                    # FIXED: Changed order keyword syntax to support your environment library version
        ...
        # Catch missing file states safely
        try:
        ...
        if portfolio is None or portfolio.empty:
            # Force-build empty structure if current_portfolio.csv is missing/empty
            portfolio = pd.DataFrame(
                columns=[
                    "symbol",
                    "entry_timestamp",
        ...
        # Sort explicitly by PQS descending (highest score priority first)
        ranked = candidates.sort_values("pqs", ascending=False)

        # Generate single real transaction timestamp string for the exact moment these positions execute
        ...
        for _, row in ranked.iterrows():
            ...
            entry = {
                "symbol": symbol,
                "entry_timestamp": execution_timestamp,  # Captures the actual execution run timestamp
                "entry_price": price,
                "quantity": qty,
                "slot_id": slot_id,
